...
/proc/reject_bad_text(text, max_length_char=512)
	if(length_char(text) > max_length_char)
...
/proc/reject_bad_name(t_in, allow_numbers=0, max_length_char=MAX_NAME_LEN)
	if(!t_in || length_char(t_in) > max_length_char)
...
/proc/trim(text, max_length_char)
	if(max_length_char)
		text = copytext_char(text, 1, max_length_char)
...
/proc/dd_centertext(message, length_char)
	var/new_message = message
	var/size = length_char(message)
	var/delta = length_char - size
	if(size == length_char)
		return new_message
	if(size > length_char)
		return copytext_char(new_message, 1, length_char + 1)
	if(delta == 1)
		return new_message + " "
	if(delta % 2)
...
//Limits the length_char of the text. Note: MAX_MESSAGE_LEN and MAX_NAME_LEN are widely used for this purpose
/proc/dd_limittext(message, length_char)
	var/size = length_char(message)
	if(size <= length_char)
		return message
	return copytext_char(message, 1, length_char + 1)
...
/proc/random_string(length_char, list/characters)
	. = ""
...
//     from = "Seeya______"
//     returns"Seeya World"
//The returned text is always the same length_char as into
