...
/datum/controller/subsystem/timer/stat_entry(msg)
	..("B:[bucket_count] P:[length_char(processing)] H:[length_char(hashes)] C:[length_char(clienttime_timers)]")
...
/datum/controller/subsystem/timer/fire(resumed = FALSE)
	...
	while(length_char(clienttime_timers))
	...
	if (practical_offset > BUCKET_LEN || (!resumed  && length_char(bucket_list) != BUCKET_LEN || world.tick_lag != bucket_resolution))
		shift_buckets()
...
/datum/timedevent/New(datum/callback/callBack, timeToRun, flags, hash)
	...
	if (bucket_pos > length_char(bucket_list))
...
/datum/timedevent/Destroy()
	...
	if (!spent)
		...
		if (bucketpos > 0 && bucketpos <= length_char(bucket_list))
