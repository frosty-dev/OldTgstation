...
/datum/beam/proc/Draw()
	...
	for(N in 0 to length_char-1 step 32)//-1 as we want < not <=, but we want the speed of X in Y to Z and step X
		var/obj/effect/ebeam/X = new beam_type(origin_oldloc)
		X.owner = src
		elements += X

		//Assign icon, for main segments it's base_icon, for the end, it's icon+icon_state
		//cropped by a transparent box of length_char-N pixel size
		if(N+32>length_char)
			var/icon/II = new(icon, icon_state)
			II.DrawBox(null,1,(length_char-N),32,32)
			X.icon = II
		else
			X.icon = base_icon
