...
/client/proc/debug_variables(datum/D in world)
	...
	if(length_char(formatted_type) > 25)
		var/middle_point = length_char(formatted_type) / 2
	...
	<body onload='selectTextField(); updateSearch()' onkeydown='return checkreload()' onkeyup='updateSearch()'>
		<script type="text/javascript">
			...
			function updateSearch(){
				...
				for ( var j = 0; j < lis_new.length_char; ++j )
				{
					var li1 = lis\[j\];
					if (j == 0){
