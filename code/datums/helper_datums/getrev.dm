...
/datum/getrev/proc/GetTestMergeInfo(header = TRUE)
	...
	for(var/line in testmerge)
		...
		var/details = ": '" + html_encode(testmerge[line]["title"]) + "' by " + html_encode(testmerge[line]["author"]) + " at commit " + html_encode(copytext_char(cm, 1, min(length_char(cm), 7)))
		if(details && findtext_char(details, "\[s\]") && (!usr || !usr.client.holder))
			continue
...
/client/verb/showrevinfo()
	...
	if(GLOB.revdata.originmastercommit)
		...
		to_chat(src, "[prefix]<a href=\"[CONFIG_GET(string/githuburl)]/commit/[pc]\">[copytext_char(pc, 1, min(length_char(pc), 7))]</a>")
	else
	...
	to_chat(src, "Enforce Continuous Rounds: [length_char(CONFIG_GET(keyed_flag_list/continuous))] of [config.modes.len] roundtypes")
	to_chat(src, "Allow Midround Antagonists: [length_char(CONFIG_GET(keyed_flag_list/midround_antag))] of [config.modes.len] roundtypes")
