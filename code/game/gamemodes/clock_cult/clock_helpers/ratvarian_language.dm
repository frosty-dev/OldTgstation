...
/proc/add_ratvarian_regex(text)
	...
	return replacetext_char(ratvarian, 				RATVAR_MY_MATCH, 		RATVAR_MY_REPLACEMENT)
...
/proc/ratvar2text(ratvarian) //Reverts ravarian styling and rot13 in text.
	...
	text = replacetext_char(rot13(text), "`", "")
	return remove_ratvarian_regex(text)

/proc/remove_ratvarian_regex(ratvarian)
	var/text 	= replacetext_char(ratvarian, 	REVERSE_RATVAR_HYPHEN_GUA_MATCH,		REVERSE_RATVAR_HYPHEN_GUA_REPLACEMENT)
	...
	return replacetext_char(text, 				REVERSE_RATVAR_HYPHEN_OF_MATCH,			REVERSE_RATVAR_HYPHEN_OF_REPLACEMENT)
