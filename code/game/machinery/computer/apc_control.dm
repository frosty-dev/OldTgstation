/obj/machinery/computer/apc_control
	...
	var/list/filterss //For sorting the results
	var/checking_logs = 0
	var/list/logs
	var/authenticated = 0
	var/auth_id = "\[NULL\]"
...
/obj/machinery/computer/apc_control/interact(mob/living/user)
	...
	if(authenticated)
		if(!checking_logs)
			...
			dat += "<i>Filters</i><br>"
			dat += "<b>Name:</b> <a href='?src=\ref[src];name_filter=1'>[filterss["Name"] ? filterss["Name"] : "None set"]</a><br>"
			...
			for(var/A in apcs)
				...
				if(filters["Name"] && !findtext_char(APC.name, filters["Name"]) && !findtext_char(APC.area.name, filterss["Name"]))
					continue
				if(filterss["Charge Above"] && (APC.cell.charge / APC.cell.maxcharge) < filterss["Charge Above"] / 100)
					continue
				if(filterss["Charge Below"] && (APC.cell.charge / APC.cell.maxcharge) > filterss["Charge Below"] / 100)
					continue
				if(filterss["Responsive"] && !APC.aidisabled)
					continue
...
/obj/machinery/computer/apc_control/Topic(href, href_list)
	...
	if(href_list["above_filter"])
		...
		playsound(src, 'sound/machines/terminal_prompt_confirm.ogg', 50, 0)
		filterss["Charge Above"] = new_filter
	if(href_list["below_filter"])
		...
		if(new_filter)
			new_filter = Clamp(new_filter, 0, 100)
		playsound(src, 'sound/machines/terminal_prompt_confirm.ogg', 50, 0)
		filterss["Charge Below"] = new_filter
	if(href_list["access_filter"])
		if(isnull(filterss["Responsive"]))
			filterss["Responsive"] = 1
			log_activity("sorted by non-responsive APCs only")
		else
			filterss["Responsive"] = !filterss["Responsive"]
