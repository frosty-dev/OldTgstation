...
/obj/machinery/computer/scan_consolenew/proc/ShowInterface(mob/user, last_change)
	...
	switch(current_screen)
		...
		if("buffer")
			...
			if(viable_occupant)
				temp_html += "<div class='dnaBlockNumber'>1</div>"
				var/len = length_char(viable_occupant.dna.struc_enzymes)
				for(var/i=1, i<=len, i++)
...
/obj/machinery/computer/scan_consolenew/proc/scramble(input,rs,rd)
	var/length_char = length_char(input)
	...
	return num2hex(Wrap(hex2num(input)+ran, 0, 16**length_char), length_char)
