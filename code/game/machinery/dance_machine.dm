...
/datum/track
	...
	var/song_path = null
	var/song_length_char = 0
	var/song_beat = 0
	var/GBP_required = 0

/datum/track/New(name, path, length_char, beat)
	song_name = name
	song_path = path
	song_length_char = length_char
	song_beat = beat

/obj/machinery/disco/proc/add_track(file, name, length_char, beat)
	var/sound/S = file
	if(!istype(S))
		return
	if(!name)
	...
	if(!beat)
		beat = 5
	if(!length_char)
		length_char = 2400 //Unless there's a way to discern via BYOND.
	var/datum/track/T = new /datum/track(name, file, length_char, beat)
	songs += T
...
/obj/machinery/disco/interact(mob/user)
	...
	dat += "Track Length: [DisplayTimeText(selection.song_length_char)]<br><br>"
...
/obj/machinery/disco/proc/dance_setup()
	stop = world.time + selection.song_length_char
