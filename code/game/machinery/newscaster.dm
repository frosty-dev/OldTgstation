...
/obj/machinery/newscaster/attack_hand(mob/user)
	...
	if(ishuman(user) || issilicon(user))
		...
		switch(screen)
			...
			if(8)
				var/total_num=length_char(GLOB.news_network.network_channels)
				var/active_num=total_num
				var/message_num=0
				for(var/datum/newscaster/feed_channel/FC in GLOB.news_network.network_channels)
					if(!FC.censored)
						message_num += length_char(FC.messages)
					else
						active_num--
	...
	if ((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && isturf(loc))) || issilicon(usr))
		...
		if(href_list["set_channel_name"])
			...
			while (findtext_char(channel_name," ") == 1)
				channel_name = copytext_char(channel_name,2,length_char(channel_name)+1)
			updateUsrDialog()
