		...
		if(3)
			if(!isnull(A.reagents))
				if(A.reagents.reagent_list.len > 0)
					var/reagents_length_char = A.reagents.reagent_list.len
					to_chat(user, "<span class='notice'>[reagents_length_char] chemical agent[reagents_length_char > 1 ? "s" : ""] found.</span>")
