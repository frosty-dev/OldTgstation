...
/*
	...
	the scan and the irradiation kicking in is determined by the wavelength_char.

		Each scan will cause the microlaser to have a brief cooldown period. Higher intensity will increase
	the cooldown, while higher wavelength_char will decrease it.

		Wavelength_char is also slightly increased by the intensity as well.
*/
...
/obj/item/device/healthanalyzer/rad_laser
	...
	var/wavelength_char = 10 // time it takes for the radiation to kick in, in seconds
...
/obj/item/device/healthanalyzer/rad_laser/attack(mob/living/M, mob/living/user)
	...
	if(!used)
		...
		used = 1
		icon_state = "health1"
		handle_cooldown(cooldown) // splits off to handle the cooldown while handling wavelength_char
		to_chat(user, "<span class='warning'>Successfully irradiated [M].</span>")
		spawn((wavelength_char+(intensity*4))*5)
			if(M)
				if(intensity >= 5)
...
/obj/item/device/healthanalyzer/rad_laser/proc/GetCooldown()
	return round(max(10, (stealth*30 + intensity*5 - wavelength_char/4)))
...
/obj/item/device/healthanalyzer/rad_laser/Topic(href, href_list)
	...
	if(href_list["rad"])
		...
		amount += wavelength_char
		wavelength_char = max(0,(min(120,amount)))
