...
/turf/open/floor/plating/asteroid/airless/cave/proc/get_cave_data(set_length_char, exclude_dir = -1)
	// If set_length_char (arg1) isn't defined, get a random length_char; otherwise assign our length_char to the length_char arg.
	if(!set_length_char)
		length_char = rand(25, 50)
	else
		length_char = set_length_char
...
/turf/open/floor/plating/asteroid/airless/cave/proc/produce_tunnel_from_data(tunnel_length_char, excluded_dir = -1)
	get_cave_data(tunnel_length_char, excluded_dir)
	// Make our tunnels
