...
/turf/open/process_cell(fire_count)
	...
	var/adjacent_turfs_length_char = LAZYLEN(adjacent_turfs)
	var/cached_atmos_cooldown = atmos_cooldown + 1

	var/planet_atmos = planetary_atmos
	if (planet_atmos)
		adjacent_turfs_length_char++
	...
	for(var/t in adjacent_turfs)
		...
		/******************* GROUP HANDLING START *****************************************************************/
			...
			//air sharing
			if(should_share_air)
				var/difference = our_air.share(enemy_air, adjacent_turfs_length_char)
				if(difference)
					if(difference > 0)
	...
	if (planet_atmos) //share our air with the "atmosphere" "above" the turf
		...
		if(our_air.compare(G))
			...
			our_air.share(G, adjacent_turfs_length_char)
			LAST_SHARE_CHECK
