/proc/chatter(message, phomeme, atom/A)
	...
	spawn(0)
		for(var/item in letter_count)
			...
			if(isnum(item))
				var/length_char = min(item, 10)
				if (length_char == 0)
					// "verbalise" long spaces
					sleep(1)
				chatter_speak_word(A.loc, phomeme, length_char)

/proc/chatter_speak_word(loc, phomeme, length_char)
	var/path = "sound/chatter/[phomeme]_[length_char].ogg"

	playsound(loc, path,
		vol = 40, vary = 0, extrarange = 3, falloff = FALSE)

	sleep((length_char + 1) * chatter_get_sleep_multiplier(phomeme))
