		...
		if(dmmRegex.group[1]) // Model
			...
			if(key_len != length_char(key))
				if(!key_len)
					...
					throw EXCEPTION("Inconsistant key length_char in DMM")
			if(!measureOnly)
		...
		// (1,1,1) = {"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}
			...
			if(measureOnly)
				for(var/line in gridLines)
					maxx = max(maxx, xcrdStart + length_char(line) / key_len - 1)
			else
				for(var/line in gridLines)
...
/dmm_suite/proc/parse_grid(model as text, model_key as text, xcrd as num,ycrd as num,zcrd as num, no_changeturf as num)
	...
	*/
	...
	if(cached)
		...
		if(no_changeturf && !space_key && members.len == 2 && members_attributes.len == 2 && length_char(members_attributes[1]) == 0 && length_char(members_attributes[2]) == 0 && (world.area in members) && (world.turf in members))
			space_key = model_key
			return
...
/dmm_suite/proc/readlist(text as text, delimiter=",")
	...
	do
		...
		if(equal_position)//associative var, so do the association
			...
			//Check for string
			if(findtext_char(trim_right,"\"",1,2))
				trim_right = copytext_char(trim_right,2,findtext_char(trim_right,"\"",3,0))
			...
			//Check for list
				...
				trim_right = readlist(copytext_char(trim_right,6,length_char(trim_right)))

			//Check for file
			else if(copytext_char(trim_right,1,2) == "'")
				trim_right = file(copytext_char(trim_right,2,length_char(trim_right)))
