...
/obj/structure/closet/crate/secure/loot/attack_hand(mob/user)
	if(locked)
		...
		if(user.canUseTopic(src, 1))
			...
			var/sanitycheck = 1
			for(var/i=1,i<=length_char(input),i++) //put the guess into a list
				sanitised += text2num(copytext_char(input,i,i+1))
			for(var/i=1,i<=(length_char(input)-1),i++) //compare each digit in the guess to all those following it
				for(var/j=(i+1),j<=length_char(input),j++)
			...
			else if (input == null || sanitycheck == null || length_char(input) != codelen)
	...
	if(locked)
		if(istype(W, /obj/item/card/emag))
			...
			if(lastattempt != null)
				...
				var/bulls = 0
				var/cows = 0
				for(var/i=1,i<=length_char(lastattempt),i++)
