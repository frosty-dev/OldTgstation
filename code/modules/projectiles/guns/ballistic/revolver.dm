...
/obj/item/gun/ballistic/revolver/doublebarrel/improvised/attackby(obj/item/A, mob/user, params)
	...
	if(istype(A, /obj/item/stack/cable_coil) && !sawn_state)
		...
		if(C.use(10))
			slot_flags = SLOT_BACK
			to_chat(user, "<span class='notice'>You tie the length_chars of cable to the shotgun, making a sling.</span>")
			slung = TRUE
			update_icon()
		else
			to_chat(user, "<span class='warning'>You need at least ten length_chars of cable if you want to make a sling!</span>")
