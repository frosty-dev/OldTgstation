...
/proc/voice_of_god(message, mob/living/user, list/span_list, base_multiplier = 1)
	...
	for(var/V in listeners)
		...
		if(devilinfo && findtext_char(message, devilinfo.truename))
			var/start = findtext_char(message, devilinfo.truename)
			listeners = list(L) //Devil names are unique.
			power_multiplier *= 5 //if you're a devil and god himself addressed you, you fucked up
			//Cut out the name so it doesn't trigger commands
			message = copytext_char(message, 0, start)+copytext_char(message, start + length_char(devilinfo.truename), length_char(message) + 1)
			break
